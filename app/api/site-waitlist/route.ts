...
import { sendSiteWaitlistEmail } from "@/lib/lead-email";
...
export async function POST(req: Request) {
  ...
  if (!project.content.assets?.bookingFullyBooked) {
    return NextResponse.json({ error: "Waitlist is not enabled for this site." }, { status: 400 });
  }
  ...
  const sent = await sendSiteWaitlistEmail({
    to,
    siteLabel,
    firstName,
    lastName,
